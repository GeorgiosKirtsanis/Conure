...
class NextItNet_Decoder:
    ...
    def __init__(self, model_para):
        ...
        self.allitem_embeddings = tf.get_variable('allitem_embeddings', [model_para['bigemb'], self.embedding_width], initializer=tf.truncated_normal_initializer(stddev=0.02))

    def train_graph(self, is_negsample=False,ispre=True):
        self.itemseq_input = tf.placeholder('int32', [None, None], name='itemseq_input')
    ...
    def model_graph(self, itemseq_input, train=True,ispre=True):
        ...
        self.context_embedding = tf.nn.embedding_lookup(self.allitem_embeddings, context_seq, name="context_embedding")

        # positional embedding
        if self.model_para['has_positionalembedding']:
            pos_emb = self.embedding(
                tf.tile(tf.expand_dims(tf.range(tf.shape(context_seq)[1]), 0), [tf.shape(itemseq_input)[0], 1]),
        ...
        for layer_id, dilation in enumerate(model_para['dilations']):
            if ispre == True:
                dilate_input = ops.nextitnet_residual_block_withmask_pre_beforeln(dilate_input,
                                                                                  dilation,
                                                                                  layer_id,
                                                                                  residual_channels,
                ...
                dilate_input = ops.nextitnet_residual_block_withmask_fine_beforeln(dilate_input, 
                                                                                   dilation,
                                                                                   layer_id, 
                                                                                   residual_channels,
                                                                                   model_para['kernel_size'],
    ...
    def embedding(self, inputs, max_position, num_units, zero_pad=True, scale=True, l2_reg=0.0, scope="embedding", with_t=False):
        with tf.variable_scope(scope):
            lookup_table = tf.get_variable('lookup_table_position', dtype=tf.float32, shape=[max_position, num_units],
