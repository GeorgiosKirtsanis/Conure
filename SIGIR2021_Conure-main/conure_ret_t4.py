...
def main():
    ...
    for iter in range(model_para['iterations']):
        ...
        while (batch_no + 1) * batch_size < train_set.shape[0]:
            ...
            # neg_target = np.array([[random_neq(items_len, targets_len, s)] for s in pos_batch])
            neg_target = np.array([[random_neq(1, targets_len, s)] for s in pos_batch])
            # neg_target = np.array([[random_neq(0, targets_len, s)] for s in pos_batch]) #20% worse
            _, loss_out = sess.run(
                [optimizer, loss],
                feed_dict={
            ...
            if numIters % args.eval_iter == 0:
                ...
                while (batch_no_test + 1) * batch_size_test < valid_set.shape[0]:
                    ...
                    # neg_target = np.array([random_negs(items_len, targets_len, negtive_samples, s) for s in pos_batch])
                    ...
                    for i in range(top_k.shape[0]):
                        ...
                        if negtive_samples == top_k_per_batch:
                            hits.append(1.0)
                        else:
                            hits.append(0.0)
                    batch_no_test += 1
                print("-------------------------------------------------------Accuracy")
                if len(hits) != 0:
                    print("Accuracy hit_n:", sum(hits) / float(len(hits)))  # 5

            if numIters % args.save_para_every == 0:
                save_path = saver_ft.save(sess,
                                       "Data/Models/generation_model_finetune_t4/model_nextitnet_transfer_pretrain.ckpt".format(iter, numIters))
